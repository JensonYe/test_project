...
class GaussianFiller : public Filler<Dtype> {
 public:
  ...
  virtual void Fill(Blob<Dtype>* blob) {
    Dtype* data = blob->mutable_cpu_data();
    CHECK(blob->count());
	if (this->filler_param_.has_file())              //*********added by ye ...2014.05.01**********//
    {
		//read file data,and fill it in the blob...
       std::ifstream infile(this->filler_param_.file().c_str());
       int i = 0;
       Dtype *data = blob->mutable_cpu_data();
       LOG(INFO) << "Reading data from "<<this->filler_param_.file();
       while(infile>>data[i++])
       {
       }
       LOG(INFO) << i-1 << " numbers have been read.";
       infile.close();
    }
	else
	{
