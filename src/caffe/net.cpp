...
void Net<Dtype>::Init(const NetParameter& in_param) {
  ...
  for (int i = 0; i < param.layers_size(); ++i) {
	...
	const int num_param_blobs = layers_[i]->blobs().size();
	...
	//********************************************************************************//
	int param_size = 0;
	if(layer_param.type() == LayerParameter_LayerType_CONVOLUTION )
	{
		param_size = layer_param.convolution_param().param_size();
		LOG(INFO)<<"conv param num : "<<param_size;
		for(int i = 0; i < param_size; ++i) {
			LOG(INFO)<<"conv param  : "<<layer_param.convolution_param().param(i);
		}
		  
	}
	else if(layer_param.type() == LayerParameter_LayerType_INNER_PRODUCT)
	{
		param_size = layer_param.inner_product_param().param_size();
		LOG(INFO)<<"ip param num : "<<param_size;
		for(int i = 0; i < param_size; ++i) {
			LOG(INFO)<<"ip param  : "<<layer_param.inner_product_param().param(i);
		}
	}
...
void Net<Dtype>::AppendParam(const NetParameter& param, const int layer_id,
  ...
  string param_name = "";
	...
	if (layer_param.type() == LayerParameter_LayerType_CONVOLUTION)
	{
		param_size = layer_param.convolution_param().param_size();
		if(param_size)
		  param_name = layer_param.convolution_param().param(param_id);
	}
	else if(layer_param.type() == LayerParameter_LayerType_INNER_PRODUCT)
	{
		param_size = layer_param.inner_product_param().param_size();
		if(param_size)
		   param_name = layer_param.inner_product_param().param(param_id);
	}
  ...
  if (!param_size || !param_name.size() || (param_name.size() &&
    ...
    layers_[layer_id]->blobs()[param_id]->ShareData(
        *layers_[owner_layer_id]->blobs()[owner_param_id]);//just data share.not the diff....
  }
}
//************************************************************************************************//
...
void Net<Dtype>::CopyTrainedLayersFrom(const NetParameter& param) {
  ...
  for (int i = 0; i < params_.size(); ++i) {
	...
	shared_ptr<Blob<Dtype> > slave_blob = params_[i];
	shared_ptr<Blob<Dtype> > owner_blob = params_[param_owners_[i]];
	slave_blob->ShareData((*owner_blob.get()));
  }
}
...
#ifndef CPU_ONLY
    case Caffe::GPU:
	 // LOG(INFO)<< " CPU Mode for caffe_gpu_add";
      ...
      break;
#else
      LOG(FATAL) << "CPU-only Mode: cannot make GPU call."
#endif
    default:
