...
void ImageDataLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  string root_folder = "";
  if(this->layer_param_.image_data_param().has_root_fold())
  {
	  root_folder = this->layer_param_.image_data_param().root_fold();
  }
  ...
  while (infile >> filename >> label) {
    lines_.push_back(std::make_pair(root_folder+filename, label));
  }
