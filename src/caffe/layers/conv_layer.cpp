...
void ConvolutionLayer<Dtype>::SetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (this->blobs_.size() > 0) {
	...
	//*********************************added by ye  9.26******************************//
	if(this->layer_param_.convolution_param().has_top_group())
	{
		//this->layer_param_.convolution_param().localconnect().local_connect(g);
		uint32_t counter = 0;
		for (int g = 0;g < group_;g++)
		{
			uint32_t top_g = this->layer_param_.convolution_param().top_group().group_num(g);
			uint32_t bottom_g = this->layer_param_.convolution_param().bottom_group().group_num(g);
			counter += top_g*bottom_g;
		}
		this->blobs_[0].reset(new Blob<Dtype>(
			counter,1,kernel_size_,kernel_size_));
...
Dtype ConvolutionLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
  ...
  bool isLocal = this->layer_param_.convolution_param().has_top_group();
  if (isLocal) {
	  int weight_offset = 0;
	  int col_offset = 0;
	  int top_offset = 0;
	  ...
	  for (int n = 0; n < num_; ++n) {
		...
		for (int g = 0; g < group_; ++g) {
			  m_ = this->layer_param_.convolution_param().top_group().group_num(g);
			  k = this->layer_param_.convolution_param().bottom_group().group_num(g);
			  k_ = k * kernel_size_ * kernel_size_;
			  caffe_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, m_, N_, k_,
				(Dtype)1., weight + weight_offset , col_data + col_offset ,
				(Dtype)0., top_data + (*top)[0]->offset(n) + top_offset );
			  weight_offset += m_ * k_;
			  col_offset += k_ * N_;
			  top_offset += m_ * N_;
		}
  ...
  if(isLocal) {
	  ...
	  for (int n = 0; n < num_; ++n) {
		...
		for (int g = 0; g < group_; ++g) {
			...
			(Dtype)1., top_diff + top[0]->offset(n) + top_offset ,
			col_data + col_offset , (Dtype)1.,
			weight_diff + weight_offset );
			weight_offset += m_ * k_ ;
			col_offset += k_ * N_;
			top_offset += m_ * N_;
		}
		...
		if (propagate_down) {
		  for (int g = 0; g < group_; ++g) {
			  ...
			  (Dtype)1., weight + weight_offset ,
			  top_diff + top[0]->offset(n) + top_offset ,
			  (Dtype)0., col_diff + col_offset );
			  weight_offset += m_ * k_;
			  col_offset += k_ * N_;
			  top_offset += m_ * N_;
		  }
